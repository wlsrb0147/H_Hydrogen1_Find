	...
	SubShader
	{
		...
		Pass
		{
			...
			PackedVaryingsMeshToPS VertexFunction(AttributesMesh inputMesh )
			{
				...
				float2 texCoord307 = inputMesh.ase_texcoord.xy * float2( 1,1 ) + float2( 0,0 );
						...
						)
			{
				...
				float2 texCoord50 = packedInput.ase_texcoord5.xy * temp_cast_1 + float2( 0,0 );
				...
				float3 temp_cast_2 = (lerpResult77).xxx;
				
				float2 uv_Glow = packedInput.ase_texcoord5.xy * _Glow_ST.xy + _Glow_ST.zw;
				float4 temp_output_291_0 = ( _GlowColor * ( ( _Glow_Intensity + _Glow_Minimum ) * tex2D( _Glow, uv_Glow ).g ) );
				float3 hdEmission325 = ASEGetEmissionHDRColor(temp_output_291_0.rgb,ConvertEvToLuminance(temp_output_291_0.r),0.0,GetInverseCurrentExposureMultiplier());
				
				float lerpResult89 = lerp( _SmoothnessA , _SmoothnessB , 0.0);
				
				surfaceDescription.BaseColor = tex2DNode2.rgb;
				surfaceDescription.Normal = BlendNormal( UnpackNormalScale( tex2D( _Normal, uv_Normal ), 1.0f ) , unpack99 );
				surfaceDescription.BentNormal = float3( 0, 0, 1 );
				surfaceDescription.CoatMask = 0;
				surfaceDescription.Metallic = 0;

				#ifdef _MATERIAL_FEATURE_SPECULAR_COLOR
				surfaceDescription.Specular = temp_cast_2;
				#endif

				surfaceDescription.Emission = hdEmission325;
				surfaceDescription.Smoothness = ( 1.0 - lerpResult89 );
				surfaceDescription.Occlusion = 1;
				surfaceDescription.Alpha = tex2DNode2.a;

				#ifdef _ALPHATEST_ON
				surfaceDescription.AlphaClipThreshold = tex2DNode2.a;
				#endif
		...
		Pass
		{
			...
			PackedVaryingsMeshToPS VertexFunction(AttributesMesh inputMesh  )
			{
				...
				float4 tex2DNode271 = tex2Dlod( _WindNoise, float4( panner272, 0, 0.0) );
				float4 appendResult308 = (float4(tex2DNode271.g , tex2DNode271.g , 0.0 , 0.0));
				float2 texCoord307 = inputMesh.uv0.xy * float2( 1,1 ) + float2( 0,0 );
				float lerpResult306 = lerp( 0.0 , 1.0 , tex2Dlod( _WindMask, float4( texCoord307, 0, 0.0) ).g);
				#ifdef _WIND_ON
				float4 staticSwitch309 = ( ( appendResult308 * _WindIntensity ) * lerpResult306 );
				#else
			...
			float4 Frag(PackedVaryingsMeshToPS packedInput  ) : SV_Target
			{
				...
				BuiltinData builtinData;
				GlobalSurfaceDescription surfaceDescription = (GlobalSurfaceDescription)0;
				float2 uv_Diffuse = packedInput.ase_texcoord2.xy * _Diffuse_ST.xy + _Diffuse_ST.zw;
				float4 tex2DNode2 = tex2D( _Diffuse, uv_Diffuse );
				
				float2 uv_Normal = packedInput.ase_texcoord2.xy * _Normal_ST.xy + _Normal_ST.zw;
				float2 temp_cast_1 = (_NormalDetailScale).xx;
				float2 texCoord50 = packedInput.ase_texcoord2.xy * temp_cast_1 + float2( 0,0 );
				float3 unpack99 = UnpackNormalScale( tex2D( _NormalDetail, texCoord50 ), _NormalDetailIntensity );
				unpack99.z = lerp( 1, unpack99.z, saturate(_NormalDetailIntensity) );
				...
				float2 uv_Glow = packedInput.ase_texcoord2.xy * _Glow_ST.xy + _Glow_ST.zw;
				float4 temp_output_291_0 = ( _GlowColor * ( ( _Glow_Intensity + _Glow_Minimum ) * tex2D( _Glow, uv_Glow ).g ) );
				float3 hdEmission325 = ASEGetEmissionHDRColor(temp_output_291_0.rgb,ConvertEvToLuminance(temp_output_291_0.r),0.0,GetInverseCurrentExposureMultiplier());
		...
		Pass
		{
					...
					)
			{
				...
				float2 uv_Diffuse = packedInput.ase_texcoord1.xy * _Diffuse_ST.xy + _Diffuse_ST.zw;
				float4 tex2DNode2 = tex2D( _Diffuse, uv_Diffuse );
		...
		Pass
		{
						...
						)
			{
				...
				float2 temp_cast_1 = (_NormalDetailScale).xx;
				float2 texCoord50 = packedInput.ase_texcoord7.xy * temp_cast_1 + float2( 0,0 );
				float3 unpack99 = UnpackNormalScale( tex2D( _NormalDetail, texCoord50 ), _NormalDetailIntensity );
				unpack99.z = lerp( 1, unpack99.z, saturate(_NormalDetailIntensity) );
				...
				float2 uv_Glow = packedInput.ase_texcoord7.xy * _Glow_ST.xy + _Glow_ST.zw;
				float4 temp_output_291_0 = ( _GlowColor * ( ( _Glow_Intensity + _Glow_Minimum ) * tex2D( _Glow, uv_Glow ).g ) );
				float3 hdEmission325 = ASEGetEmissionHDRColor(temp_output_291_0.rgb,ConvertEvToLuminance(temp_output_291_0.r),0.0,GetInverseCurrentExposureMultiplier());
...
WireConnection;314;5;77;0
WireConnection;314;6;325;0
WireConnection;314;7;313;0
WireConnection;314;9;2;4
WireConnection;314;10;2;4
WireConnection;314;11;309;0
WireConnection;325;0;291;0
WireConnection;325;1;291;0
ASEEND*/
//CHKSM=D02FFD6C0453D4493D2B36D0341925E8897FEE67